<?php
function reset_rewards_for_player($user_id)
{
    global $conn; // Use the $conn object from the global scope

    // Check if the user already has rewards
    $stmt = $conn->prepare("SELECT * FROM rewards WHERE user_id = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();

    // If the user already has rewards, return early
    if ($result->num_rows > 0) {
        return;
    }

    // Get the user from the player table
    $stmt = $conn->prepare("SELECT player_id, skill FROM player WHERE player_id = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    // Generate 7 random rewards for the player
    for ($day = 1; $day <= 7; $day++) {
        ...
        $cumulative = 0.0;
        $reward_type = array_keys($rewards)[0]; // Default to the first reward type
